            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-content">
                        <div class="card-body">
                            <form class="form" method="post" enctype="multipart/form-data" action="{{route(currentUser().'.proPhoto.update',encryptor('encrypt',$photo->id))}}">
                                @csrf
                                @method('PATCH')
                                <div class="row">
                                    <div class="col-8 offset-2">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control form-select" name="property" id="property" required>
                                                ...
                                                @forelse($prop as $d)
                                                    <option value="{{$d->id}}" {{ old('property',$photo->property_id)==$d->id?"selected":""}}> {{ $d->name}}</option>
                                                @empty
